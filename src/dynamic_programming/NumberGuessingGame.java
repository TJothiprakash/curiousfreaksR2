...
Input: n = 1
Output: 0
Explanation: There is only one possible number,
 so you can guess 1 and not have to pay anything.
Example 3:
...
public class NumberGuessingGame {
    static void main() {
    int ans = new NumberGuessingGame().getMoneyAmount(200);
        System.out.println("ans = " + ans);
    }
    public int getMoneyAmount(int n) {
        // dp[i][j] = minimum money to guarantee a win for range [i..j]
        int[][] dp = new int[n + 2][n + 2]; // use n+2 to avoid index issues

        // length of interval
        for (int len = 2; len <= n; len++) {  // intervals of size 2 to n
            for (int start = 1; start <= n - len + 1; start++) {
                int end = start + len - 1;
                dp[start][end] = Integer.MAX_VALUE;

                // try every possible guess in this interval
